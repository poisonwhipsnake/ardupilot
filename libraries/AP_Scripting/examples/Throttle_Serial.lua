...
assert(param:add_table(PARAM_TABLE_KEY, "LUA_THR_", 2), 'could not add param table')
...
local gain = Parameter("LUA_THR_GAIN")
local param2 = Parameter("LUA_THR_TEST")
...
function update()
    ...
    if (throttleDemand) then
        local throttle =  math.floor(math.abs(throttleDemand)* 250);
        if throttle > 250 then
            throttle = 250
        end
        if throttle < 0 then
            throttle = 0
