...
/*
  constructor
 */
AP_Baro_Encoder::AP_Baro_Encoder(AP_Baro &baro, AP_HAL::OwnPtr<AP_HAL::Device> dev, enum MS56XX_TYPE ms56xx_type)
...
AP_Baro_Backend *AP_Baro_Encoder::probe(AP_Baro &baro,
                                       AP_HAL::OwnPtr<AP_HAL::Device> dev,
                                       enum MS56XX_TYPE ms56xx_type)
{
    gcs().send_text(MAV_SEVERITY_INFO, "dev?");
    if (!dev) {
        return nullptr;
    }
    gcs().send_text(MAV_SEVERITY_INFO, "in probe");
    AP_Baro_Encoder *sensor = new AP_Baro_Encoder(baro, std::move(dev), ms56xx_type);
    gcs().send_text(MAV_SEVERITY_INFO, "!sensor? %d", !sensor);
...
bool AP_Baro_Encoder::_init()
{
    gcs().send_text(MAV_SEVERITY_INFO, "init begin");
    if (!_dev) {
        return false;
    }

    gcs().send_text(MAV_SEVERITY_INFO, "Using encoder");
    ...
    // SPI guidelines from datasheet

    // read the setting1 reg
    uint32_t address = 0x0018;
    uint32_t rw = 1; // 1: read, 0: write
    
    uint32_t data = (address << 1) | (rw);

    // if data is odd, add parity bit
    uint32_t parity_data = data;
    bool parity = true;
    while(parity_data) {
        parity = !parity;
    ...
    gcs().send_text(MAV_SEVERITY_INFO, "data: %ld", data);
    ...
    for (int i = 0; i < 2; i++) {
        gcs().send_text(MAV_SEVERITY_INFO, "buf[%d]: %d\n", i, buf[i]);
    }
    ...
    _dev->register_periodic_callback(10 * AP_USEC_PER_MSEC,
                                     FUNCTOR_BIND_MEMBER(&AP_Baro_Encoder::_timer, void));
    return true;
}

uint16_t AP_Baro_Encoder::_read_prom_word(uint8_t word)
{
...
uint32_t AP_Baro_Encoder::_read_adc()
{
    uint8_t val[3];
...
bool AP_Baro_Encoder::_read_prom_5611(uint16_t prom[8])
{
    /*
...
bool AP_Baro_Encoder::_read_prom_5637(uint16_t prom[8])
{
    /*
 ...
 * pressure.
*/
void AP_Baro_Encoder::_timer(void)
{
    uint8_t next_cmd;
    uint8_t next_state;
...
void AP_Baro_Encoder::_update_and_wrap_accumulator(uint32_t *accum, uint32_t val,
...
void AP_Baro_Encoder::_calculate_5611()
{
    float dT;
    float TEMP;
    float OFF;
...
// Calculate Temperature and compensated Pressure in real units (Celsius degrees*100, mbar*100).
void AP_Baro_Encoder::_calculate_5607()
{
    float dT;
    float TEMP;
    float OFF;
...
// Calculate Temperature and compensated Pressure in real units (Celsius degrees*100, mbar*100).
void AP_Baro_Encoder::_calculate_5637()
{
    int32_t dT, TEMP;
    int64_t OFF, SENS;
...
// Calculate Temperature and compensated Pressure in real units (Celsius degrees*100, mbar*100).
void AP_Baro_Encoder::_calculate_5837()
{
    int32_t dT, TEMP;
    int64_t OFF, SENS;
    int32_t raw_pressure = _D1;
