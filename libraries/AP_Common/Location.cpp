...
void Location::calc_turn_centre(
    ...
    Vector2f &out_turn_start_dist)
{
    ...
    float turn_distance = tanf(abs(_ground_turn_angle/2))*turn_radius;                      // Linear distance away from WP to begin the turn to maintain the parameterised circular ground track of radius 'turn_radius'
    ...
    Vector2f centre_from_turn_WP = -Current_Track*turn_distance + Vector2f(Current_Track.y,-Current_Track.x)*turn_radius;   // Centre of the ground frame turn circle
    ...
    if(_ground_turn_angle>0){
        centre_from_turn_WP = -Current_Track*turn_distance + Vector2f(-Current_Track.y,Current_Track.x)*turn_radius;
