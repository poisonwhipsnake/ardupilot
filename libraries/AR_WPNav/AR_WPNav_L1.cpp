...
const AP_Param::GroupInfo AR_WPNav_L1::var_info[] = {
    ...
    AP_GROUPINFO("TURN_PER", 3, AR_WPNav_L1, _L1_Turning_Period, 20.0f),

    AP_GROUPINFO("EXIT_PCT", 4, AR_WPNav_L1, _L1_Exit_Percent, 1.0f),

    AP_GROUPINFO("TURN_RAD",5, AR_WPNav_L1, _turn_radius, 10.0),

    AP_GROUPINFO("TRC",6, AR_WPNav_L1, _turn_radius_compensation, 0.0),

    AP_GROUPINFO("TSO",7, AR_WPNav_L1, _turn_start_offset, 0.0),

    AP_GROUPINFO("T_ERROR",8, AR_WPNav_L1, _max_auto_point_distance, 1000.0f),

    AP_GROUPINFO("STR_MAX", 9, AR_WPNav_L1, _steering_angle_max_param, 30.0f),

    AP_GROUPINFO("STR_VEL", 10, AR_WPNav_L1, _steering_angle_velocity_param, 20.0f),

    AP_GROUPINFO("STR_ACCEL", 11, AR_WPNav_L1, _steering_angle_acceleration_param, 50.0f),

    AP_GROUPINFO("STR_WB", 12, AR_WPNav_L1, _steering_wheelbase, 0.4f),

    AP_GROUPINFO("SPD_MAX", 13, AR_WPNav_L1, _speed_max_param, 2.0f),

    AP_GROUPINFO("TURN_G",14, AR_WPNav_L1, _turn_lateral_G , 0.1f),

    AP_GROUPINFO("XTR_MAX",15, AR_WPNav_L1, crosstrack_max, 1.0f),

    AP_GROUPINFO("XTR_SPD",16, AR_WPNav_L1, crosstrack_low_speed, 1.0f),

    AP_GROUPINFO("MIN_L1",17, AR_WPNav_L1, _min_L1, 10.0f),

    AP_GROUPINFO("RATE_Hz",18, AR_WPNav_L1, _rate_filter_hz, 10.0f),

    AP_GROUPINFO("STR_A_SF",19, AR_WPNav_L1, _steering_angle_accel_safety_factor, 2.0f),

    AP_GROUPINFO("L1_GS_EXP",20 , AR_WPNav_L1, _L1_Groundspeed_Exponent, 1.0f),
...
void AR_WPNav_L1::update(float dt)
{
    ...
    update_waypoint( prev_waypoint,current_waypoint,_min_L1);

    Vector2f turnDistance = turn_distance(prev_waypoint,current_loc, current_waypoint,next_waypoint, 5.0f);

    float distance_to_waypoint = current_loc.get_distance_NE(current_waypoint).length() ;
...
float AR_WPNav_L1::nav_steering_angle(float groundspeed, float wheelbase, float _steering_angle_max, float _steering_angle_max_rate, float _steering_angle_max_accel, float turn_radius) 
{
    ...
    if( (ground_turn_angle_remaining*auto_turn_clockwise) / theta < _L1_Exit_Percent && !_initial_turn_complete ) {
        _initial_turn_complete = true;
        auto_turn_centre.zero();
        _L1_xtrack_i = 0.0f;
...
Vector2f AR_WPNav_L1::turn_distance(const struct Location &previous_wp, const struct Location &current_loc, const struct Location &turn_WP, const struct Location &next_WP, float _trimspeed) //const
{
    ...
    Location::calc_turn_centre(previous_wp, current_loc, turn_WP, next_WP, waypoint_radius, _turn_start_offset,
                                     potential_turn_centre, potential_turn_vector, potential_turn_direction, heading_1, heading_2, returnValue);

    if (initial_turn_complete()){
        auto_turn_clockwise = potential_turn_direction; // direction of auto_turn. Positive is clockwise
        ...
        auto_turn_exit_track = heading_2;  // set auto_turn_exit_track to be the linear track between the WP to be completed and the WP following
    }
    // If you are in a turn. The exit track is the angle of the "Current Track" that you are aiming to turn onto.
        ...
        auto_turn_exit_track = heading_1;  // set auto_turn_exit_track to be the linear track between the WP we just ticked off and are in the turn, and the WP following
    }
...
void AR_WPNav_L1::update_waypoint(const struct Location &prev_WP, const struct Location &next_WP, float dist_min)
{
    ...
    current_waypoint.clone(next_WP);
    prev_waypoint.clone(prev_WP);
    ...
    if (!initial_turn_complete() && canLoiterTurn){
        ...
        update_loiter(auto_turn_centre,prev_waypoint_radius-_turn_radius_compensation,auto_turn_clockwise);
        _current_nav_type = 1;
    }
    else{
...
bool AR_WPNav_L1::set_desired_location(const Location& destination, Location next_destination)
{
    ...
    if (current_waypoint.is_zero()){
        prev_waypoint.clone(_current_loc);
        prev_waypoint_radius = _turn_radius;
        
    }
    else{
        prev_waypoint.clone(current_waypoint);
        if (prev_waypoint.get_alt_frame() == Location::AltFrame::ABOVE_HOME)
        {
            prev_waypoint_radius = prev_waypoint.alt/100;
        }
        else
        {
            prev_waypoint_radius = _turn_radius;
    ...
    waypoint_radius = _turn_radius;


    current_waypoint.clone(_destination);
    next_waypoint.clone(next_destination);

    if (current_waypoint.get_alt_frame() == Location::AltFrame::ABOVE_HOME)
    {
        waypoint_radius = current_waypoint.alt/100;
    }
...
void AR_WPNav_L1::update_loiter(const struct Location &center_WP, float radius, int8_t loiter_direction)
{
    ...
    float omega = (6.2832f / _L1_Turning_Period);
